...
## Additional subsets (FD002, FD003, FD004)

`data_extra/FD002`, `data_extra/FD003` and `data_extra/FD004` contain the corresponding NASA C-MAPSS files in the same format. The replication
in Section 4.8 of the paper (centralized, FedAvg, gossip, local-only; seeds 11, 22, 33; unchanged FD001 protocol) is run with

```bash
for d in FD002 FD003 FD004; do for s in 11 22 33; do for m in centralized fedavg gossip local; do
